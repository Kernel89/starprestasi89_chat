...
const SQ_LEVELS = [
  { name: 'Tinggi', min: 148, color: '#059669', label: 'Spiritualitas Matang' },
  { name: 'Moderat', min: 94, color: '#10b981', label: 'Spiritualitas Berkembang' },
  { name: 'Rendah', min: 0, color: '#f59e0b', label: 'Butuh Penguatan' }
];

const SqReport: React.FC<SqReportProps> = ({ submissions, setSubmissions, students, rombels, assignments, schoolProfile, counselorProfile, notify, userRole }) => {
  ...
  const sqSubmissions = useMemo(() => {
    return submissions.filter(s => {
        const assign = assignments.find(a => a.id === s.assignmentId);
        return s.sqScore !== undefined && (assign?.type === 'SQ' || !assign);
    });
  }, [submissions, assignments]);

  const filteredResults = useMemo(() => {
    let list = sqSubmissions.map(sub => {
      const student = students.find(s => s.id === sub.studentId);
      const score = sub.sqScore || 0;
    ...
    return list.sort((a, b) => (b.sqScore || 0) - (a.sqScore || 0));
  }, [sqSubmissions, students, selectedClass, selectedGrade]);
  ...
  const stats = useMemo(() => {
    ...
    return [
      { name: 'Tinggi', value: counts.Tinggi, color: '#059669' },
      { name: 'Moderat', value: counts.Moderat, color: '#10b981' },
      { name: 'Rendah', value: counts.Rendah, color: '#f59e0b' }
    ];
  }, [filteredResults]);
  ...
  const handlePrintPDF = () => {
    ...
    doc.text(`LAPORAN HASIL TES KECERDASAN SPIRITUAL (SQ)`, 148.5, startY + 5, { align: 'center' });
    doc.setFontSize(10); doc.setFont("times", "normal");
    doc.text(periodStr, 148.5, startY + 10, { align: 'center' });

    autoTable(doc, {
      startY: startY + 20,
      head: [['No', 'Nama Lengkap Siswa', 'Tgl Tes', 'Skor SQ', 'Kategori', 'Keterangan']],
      body: filteredResults.map((r, i) => [
        ...
        r.sqScore || '0',
        r.level || '-',
        SQ_LEVELS.find(l => l.name === r.level)?.label || '-'
      ]),
    ...
    doc.save(`Laporan_SQ_${selectedClass.replace(/\s+/g, '_')}.pdf`);
    notify("Laporan SQ berhasil diunduh.");
  };

  const handleDelete = async (id: string) => {
    if (confirm("Apakah Anda yakin ingin menghapus hasil tes ini? Siswa yang bersangkutan akan dapat mengulang kembali tes SQ.")) {
        if (setSubmissions) {
            setSubmissions(prev => prev.filter(s => !(s.id === id && s.sqScore !== undefined)));
            await deleteFromCloud('star_sqSubmissions', id);
            notify("Hasil tes SQ berhasil dihapus.", "success");
        }
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Laporan SQ Siswa</h2>
          <p className="text-slate-500 text-sm font-medium">Rekapitulasi indeks kecerdasan spiritual untuk bimbingan nilai dan karakter.</p>
        </div>
        <div className="flex gap-2">
            {!isPrincipal && (
            <button onClick={handlePrintPDF} className="bg-emerald-600 text-white px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-100 transition-all flex items-center justify-center gap-3 active:scale-95">
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 3v18M8 6l8 12M16 6l-8 12"/></svg>
                    </div>
                    <div>
                ...
                <div className="text-right">
                    ...
                    <p className="text-2xl font-black text-emerald-600">{filteredResults.length} Siswa</p>
                </div>
            </div>

            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col h-[400px]">
                <div className="mb-6">
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Distribusi Kategori SQ</h3>
                </div>
                <div className="flex-1 w-full min-h-0">
                    {filteredResults.length > 0 ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-300 italic text-xs font-bold">Belum ada data pengisian SQ.</div>
                    )}
                </div>
            </div>
         </div>

         <div className="lg:col-span-4 bg-emerald-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-center">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] mb-4 opacity-50">SQ Matrix</h4>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Detail Hasil Skor Spiritual</h3>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="bg-white border-b">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase text-center">Skor SQ</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredResults.map((res) => (
                <tr key={res.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-8 py-5 text-center font-black text-emerald-600 text-lg">{res.sqScore}</td>
                  <td className="px-8 py-5 text-center">
                    <span className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase text-white`} style={{ backgroundColor: SQ_LEVELS.find(l => l.name === res.level)?.color }}>
                        {res.level}
                    </span>
                  </td>
