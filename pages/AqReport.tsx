...
const AQ_LEVELS = [
  { name: 'Tinggi', min: 148, color: '#0891b2', label: 'Climber (Pejuang)' },
  { name: 'Moderat', min: 94, color: '#14b8a6', label: 'Camper (Bertahan)' },
  { name: 'Rendah', min: 0, color: '#94a3b8', label: 'Quitter (Menyerah)' }
];

const AqReport: React.FC<AqReportProps> = ({ submissions, setSubmissions, students, rombels, assignments, schoolProfile, counselorProfile, notify, userRole }) => {
  ...
  const aqSubmissions = useMemo(() => {
    return submissions.filter(s => {
        const assign = assignments.find(a => a.id === s.assignmentId);
        return s.aqScore !== undefined && (assign?.type === 'AQ' || !assign);
    });
  }, [submissions, assignments]);

  const filteredResults = useMemo(() => {
    let list = aqSubmissions.map(sub => {
      const student = students.find(s => s.id === sub.studentId);
      const score = sub.aqScore || 0;
      const level = score >= 148 ? 'Tinggi' : score >= 94 ? 'Moderat' : 'Rendah';
      const coreLabel = score >= 148 ? 'Climber' : score >= 94 ? 'Camper' : 'Quitter';
      return {
        ...sub,
    ...
    return list.sort((a, b) => (b.aqScore || 0) - (a.aqScore || 0));
  }, [aqSubmissions, students, selectedClass, selectedGrade]);
  ...
  const stats = useMemo(() => {
    ...
    return [
      { name: 'Climber', value: counts.Tinggi, color: '#0891b2' },
      { name: 'Camper', value: counts.Moderat, color: '#14b8a6' },
      { name: 'Quitter', value: counts.Rendah, color: '#94a3b8' }
    ];
  }, [filteredResults]);
  ...
  const handlePrintPDF = () => {
    ...
    doc.text(`LAPORAN HASIL TES ADVERSITY QUOTIENT (AQ) - DAYA JUANG`, 148.5, startY + 5, { align: 'center' });
    doc.setFontSize(10); doc.setFont("times", "normal");
    doc.text(periodStr, 148.5, startY + 10, { align: 'center' });

    autoTable(doc, {
      startY: startY + 20,
      head: [['No', 'Nama Lengkap Siswa', 'Tgl Tes', 'Skor AQ', 'Kategori']],
    ...
    doc.save(`Laporan_AQ_${selectedClass.replace(/\s+/g, '_')}.pdf`);
    notify("Laporan AQ berhasil diunduh.");
  };

  const handleDelete = async (id: string) => {
    if (confirm("Apakah Anda yakin ingin menghapus hasil tes ini? Siswa yang bersangkutan akan dapat mengulang kembali tes AQ.")) {
        if (setSubmissions) {
            setSubmissions(prev => prev.filter(s => !(s.id === id && s.aqScore !== undefined)));
            await deleteFromCloud('star_aqSubmissions', id);
            notify("Hasil tes AQ berhasil dihapus.", "success");
        }
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Laporan AQ (Daya Juang)</h2>
          <p className="text-slate-500 text-sm font-medium">Pemetaan ketahanan mental siswa menghadapi hambatan dan kesulitan.</p>
        </div>
        <div className="flex gap-2">
            {!isPrincipal && (
            <button onClick={handlePrintPDF} className="bg-cyan-600 text-white px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-cyan-700 shadow-xl shadow-cyan-100 transition-all flex items-center justify-center gap-3 active:scale-95">
         ...
         <div className="lg:col-span-8 space-y-8">
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Responden AQ</p>
                    <p className="text-2xl font-black text-cyan-600">{filteredResults.length} Siswa</p>
                </div>
            </div>

            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col h-[400px]">
                <div className="mb-6">
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Profil Ketahanan Mental Kelas</h3>
                </div>
                <div className="flex-1 w-full min-h-0">
                    {filteredResults.length > 0 ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-300 italic text-xs font-bold">Belum ada data pengisian AQ.</div>
                    )}
                </div>
            </div>
         </div>

         <div className="lg:col-span-4 bg-cyan-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-center">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] mb-4 opacity-50">CORE Methodology</h4>
            ...
            <p className="mt-8 text-xs font-medium italic opacity-80 leading-relaxed">
                "Daya juang diukur berdasarkan kontrol diri, rasa memiliki masalah, jangkauan dampak, dan daya tahan waktu."
            </p>
         </div>
      </div>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Detail Klasifikasi AQ Siswa</h3>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="bg-white border-b">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase text-center">Skor AQ</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase text-center">Tipe CORE</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredResults.map((res) => (
                <tr key={res.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-5"><p className="font-bold text-slate-800 text-sm">{res.studentName}</p></td>
                  <td className="px-8 py-5 text-center font-black text-cyan-600 text-lg">{res.aqScore}</td>
                  <td className="px-8 py-5 text-center">
                    <span className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase text-white`} style={{ backgroundColor: AQ_LEVELS.find(l => l.name === res.level)?.color }}>
                        {res.coreLabel}
                    </span>
                  </td>
